...
_GEO_HTML = """
<div style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif; display:flex; flex-direction:column; align-items:center; justify-content:center; min-height:4rem;">
  <button type="button" onclick="getLocation()" style="
    background: linear-gradient(180deg, __PRIMARY_LIGHT__ 0%, __PRIMARY__ 100%);
    color: white; border: none; padding: 0.65rem 1.35rem;
    border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 1rem;
    letter-spacing: 0.02em;
    box-shadow: 0 2px 10px rgba(15, 92, 107, 0.28);
  ">Check location &amp; status</button>
  <p id="geo-msg" style="margin-top:0.55rem;color:__TEXT_MUTED__;font-size:0.88rem;max-width:36rem;text-align:center;"></p>
</div>
<script>
var geoOptions = {
...
""".replace("__PRIMARY_LIGHT__", COLORS["primary_light"]).replace(
...
def _assistant_header() -> None:
    ...
    st.caption("Powered by **Google Gemini** (API key on server). Not a substitute for official alerts.")
...
def _inject_chat_layout_css() -> None:
    ...
    pl, pr, pd = COLORS["primary_light"], COLORS["primary"], COLORS["primary_dark"]
    st.markdown(
        f"""
<style>
  ...
  section[data-testid="stMain"] .block-container .stButton > button {{
    background: linear-gradient(180deg, {pl} 0%, {pr} 100%) !important;
    border: 1px solid {pd} !important;
